...
def _force_additional_properties_false(node: Any) -> None:
    """Cerebras' strict structured-output mode requires every object in the
    JSON schema to set `additionalProperties: false` (see
    https://inference-docs.cerebras.ai/capabilities/structured-outputs).
    Pydantic's `model_json_schema()` doesn't set this by default, so we patch
    it in — recursively, since a schema can nest objects via `$defs`/`$ref`.
    """
...
class CerebrasProvider(ModelProvider):
    """ModelProvider adapter for Cerebras Inference, via its OpenAI-compatible
    API. Kept as an inactive reference implementation — not selected by
    ...
    def _error_response(
        ...
        prefix: str = "Cerebras call failed",
    ) -> GatewayResponse[T]:
